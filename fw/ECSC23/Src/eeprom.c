...
HAL_StatusTypeDef eeprom_write(uint8_t *src, uint8_t i2c_addr, uint16_t reg_addr, uint16_t nbytes) {
  ...
  for (int i = 0; i < nbytes; i += 8) {
    res |= HAL_I2C_Mem_Write(&hi2c1, (uint16_t) (i2c_addr << 1), i, I2C_MEMADD_SIZE_8BIT, &src[i], 8, I2C_TIMEOUT);
    int tick = HAL_GetTick()+10; //Delay a little bit.
...
HAL_StatusTypeDef eeprom_restore() {
  ...
  uint8_t eeprom1_restore_data[EEPROM_LEN] = {
    ...
    0x25, 0x0C, 0xEC, 0xE7, 0x73, 0x10, 0x97, 0x81, 0xEA, 0xD0, 0x0D, 0x2B, 0xAD, 0xEA, 0xD7, 0xDF, // chall3 here (DO OD 2B AD)
    0xD1, 0x9F, 0xF2, 0xD8, 0x3E, 0x56, 0x1A, 0x4E, 0x9E, 0x42, 0x8F, 0xCB, 0x30, 0xA8, 0x70, 0xDF,
    ...
    0xAE, 0xE8, 0xDE, 0x56, 0xAF, 0xF0, 0xEF, 0x73, 0x13, 0xEF, 0xA3, 0x06, 0xD8, 0xB8, 0x0B, 0xDC, // chall1 key
  ...
  HAL_GPIO_WritePin(EEPROM1_WC_GPIO_Port, EEPROM1_WC_Pin, GPIO_PIN_RESET);
  HAL_GPIO_WritePin(EEPROM2_WC_GPIO_Port, EEPROM2_WC_Pin, GPIO_PIN_RESET);
  ...
  uint8_t eeprom2_restore_data[EEPROM_LEN] = {
    ...
    0xC2, 0x8C, 0x91, 0xC4, 0x77, 0x89, 0x71, 0x22, 0x54, 0x64, 0x53, 0x00, 0xCE, 0x4F, 0x64, 0x7F, // chall2 key
  ...
  HAL_GPIO_WritePin(EEPROM2_WC_GPIO_Port, EEPROM2_WC_Pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(EEPROM1_WC_GPIO_Port, EEPROM1_WC_Pin, GPIO_PIN_SET);
